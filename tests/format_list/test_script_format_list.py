...
class MyTestCase(unittest.TestCase):
    ...
    def tearDown(self):
        ...
        if exists(join('test_data', f'combined_format_data_{today}.csv')):
            remove(join('test_data', f'combined_format_data_{today}.csv'))

    def test_correct(self):
        script = join(getcwd(), '..', '..', 'format_list.py')
        input_directory = 'test_data_space'
        run(f'python "{script}" "{input_directory}"', shell=True, stdout=PIPE)

        today = date.today().strftime('%Y-%m-%d')
        result = csv_to_list(join('test_data_space', f'combined_format_data_{today}.csv'))
    ...
    def test_error(self):
        ...
        script = join(getcwd(), '..', '..', 'format_list.py')
        input_directory = 'test_data_error'
        ...
        expected = "Provided input_directory 'test_data_error' does not exist\r\n"
