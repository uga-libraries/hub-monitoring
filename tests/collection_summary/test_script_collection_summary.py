...
class MyTestCase(unittest.TestCase):
    ...
    def tearDown(self):
        ...
        outputs = [os.path.join('test_data', 'Hargrett_Hub', 'Born-digital', f'hub-accession-summary_{today}.csv'),
                   os.path.join('test_data', 'Hargrett_Hub', 'Born-digital', f'hub-collection-summary_{today}.csv'),
                   os.path.join('test_data', 'Russell_Hub', 'born-digital', f'hub-accession-summary_{today}.csv'),
                   os.path.join('test_data', 'Russell_Hub', 'born-digital', f'hub-collection-summary_{today}.csv')]
    ...
    def test_error(self):
        ...
        script = os.path.join(getcwd(), '..', '..', 'collection_summary.py')
    ...
    def test_hargrett(self):
        ...
        script = os.path.join(getcwd(), '..', '..', 'collection_summary.py')
        input_directory = os.path.join(getcwd(), 'test_data', 'Hargrett_Hub', 'Born-digital')
        subprocess.run(f'python "{script}" "{input_directory}"', shell=True, stdout=subprocess.PIPE)

        # Tests the contents of the accession report.
        acc_path = os.path.join(input_directory, f"hub-accession-summary_{datetime.today().strftime('%Y-%m-%d')}.csv")
        ...
        coll_path = os.path.join(input_directory, f"hub-collection-summary_{datetime.today().strftime('%Y-%m-%d')}.csv")
