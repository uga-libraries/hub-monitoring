...
class MyTestCase(unittest.TestCase):
    ...
    def test_all_risks_once(self):
        ...
        acc_path = join('test_data', 'Russell_Hub', 'backlog', 'rbrl002', '2022-01-er')
        risk = get_risk(acc_path)
        expected = [1, 1, 1, 1]
        self.assertEqual(risk, expected, "Problem with test for all risks once")

    def test_all_risks_repeated(self):
        """Test for when there are a different number of files for each risk level"""
        acc_path = join('test_data', 'Russell_Hub', 'backlog', 'rbrl002', '2022-02-er')
        risk = get_risk(acc_path)
        expected = [5, 4, 3, 2]
        self.assertEqual(risk, expected, "Problem with test for all risks repeated")

    def test_duplicates(self):
        """Test for when all files are in the risk CSV more than once, with the same risk level"""
        acc_path = join('test_data', 'Russell_Hub', 'backlog', 'rbrl002', '2021-11-er')
        risk = get_risk(acc_path)
        expected = [1, 0, 1, 2]
        self.assertEqual(risk, expected, "Problem with test for duplicates, all files")
    ...
    def test_duplicates_mix(self):
        ...
        acc_path = join('test_data', 'Russell_Hub', 'backlog', 'rbrl002', '2021-12-er')
        risk = get_risk(acc_path)
        expected = [2, 1, 2, 3]
        self.assertEqual(risk, expected, "Problem with test for duplicates, some files")

    def test_duplicates_partial(self):
        """Test for when some files are in the risk CSV more than once, with the same risk level"""
        acc_path = join('test_data', 'Russell_Hub', 'backlog', 'rbrl002', '2021-13-er')
        risk = get_risk(acc_path)
        expected = [1, 1, 0, 2]
        self.assertEqual(risk, expected, "Problem with test for duplicates, some files")
    ...
    def test_no_risk_csv(self):
        ...
        acc_path = join('test_data', 'Russell_Hub', 'backlog', 'rbrl002', '2021-40-er')
        risk = get_risk(acc_path)
        expected = [0, 0, 0, 0]
    ...
    def test_one_risk(self):
        ...
        acc_path = join('test_data', 'Russell_Hub', 'backlog', 'rbrl002', '2022-03-er')
        risk = get_risk(acc_path)
        expected = [0, 0, 0, 1]
        self.assertEqual(risk, expected, "Problem with test for one risk once")

    def test_one_risk_repeated(self):
        """Test for when there is one file with multiple risk levels"""
        acc_path = join('test_data', 'Russell_Hub', 'backlog', 'rbrl002', '2022-04-er')
        risk = get_risk(acc_path)
        expected = [0, 0, 1, 1]
        self.assertEqual(risk, expected, "Problem with test for one risk repeated")
    ...
    def test_two_risks(self):
        ...
        acc_path = join('test_data', 'Russell_Hub', 'backlog', 'rbrl002', '2022-05-er')
        risk = get_risk(acc_path)
        expected = [0, 2, 3, 0]
